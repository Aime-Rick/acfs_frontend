...
const Settings: React.FC = () => {
  ...
  const saveApiKey = async (keyName: string) => {
    const value = apiKeys[keyName as keyof ApiKeys];
    if (!value) {
      toast.error('Veuillez entrer une valeur pour la clé API');
      return;
    }

    setSavingStatus(prev => ({ ...prev, [keyName]: 'saving' }));
    try {
      const response = await apiService.setEnvironmentVariable(keyName, value);
      if (response.status === 'success') {
        setSavingStatus(prev => ({ ...prev, [keyName]: 'success' }));
        toast.success(`${keyName} sauvegardée avec succès !`);
        
        // Reset status after 3 seconds
        setTimeout(() => {
          setSavingStatus(prev => ({ ...prev, [keyName]: 'idle' }));
        }, 3000);
      } else {
        throw new Error(response.message || 'Failed to save API key');
      }
    } catch (error) {
      setSavingStatus(prev => ({ ...prev, [keyName]: 'error' }));
      toast.error(`Erreur lors de la sauvegarde de ${keyName}`);
  ...
  const renderApiKeyField = (keyName: string, label: string, placeholder: string) => {
    const status = savingStatus[keyName] || 'idle';
      ...
      <div key={keyName} className="space-y-2">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            disabled={!apiKeys[keyName as keyof ApiKeys] || status === 'saving'}
        ...
        {status === 'success' && <p className="text-xs text-green-600 dark:text-green-400">Clé API sauvegardée</p>}
        {status === 'error' && <p className="text-xs text-red-600 dark:text-red-400">Erreur de sauvegarde</p>}
      </div>
    );
  };
