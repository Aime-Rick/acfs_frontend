...
class ApiService {
  ...
  // Clean mission names for bucket paths and file operations
  private cleanMissionName(missionName: string): string {
    return missionName
      .normalize('NFD')
    ...
    try {
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          local_file_path: `${reportPath.split('/').pop()}`
        }),
      });
  ...
  async deepResearch(context: string, problematique: string, objectives: string[], missionName: string) {
    return this.request('/research', {
      ...
      body: JSON.stringify({
        ...
        problematique,
        objectives,
        mission_name: this.cleanMissionName(missionName),
      }),
    });
  }
  ...
  async runEDA(missionName: string, file: File) {
    ...
    formData.append('mission_name', this.cleanMissionName(missionName));
  ...
  async saveReportToPdf(markdownContent: string, reportType: string, fileName: string, missionName: string) {
    return this.request('/report/pdf', {
      method: 'POST',
      body: JSON.stringify({
        markdown_content: markdownContent,
  ...
  async uploadMultipleFiles(missionName: string, files: File[]) {
    const formData = new FormData();
    formData.append('mission_name', this.cleanMissionName(missionName));
    files.forEach(file => {
      formData.append('files', file);
    });
    return this.privateUploadRequest('/files/multi-uploads', formData);
  }
  ...
  async setEnvironmentVariable(key: string, value: string) {
    ...
    return this.privateUploadRequest('/env/set', formData);
  }
}
