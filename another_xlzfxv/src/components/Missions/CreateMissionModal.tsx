  ...
  const handleGenerateCadrage = async () => {
    ...
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        step2Form.setValue('context', scopeData.context);
        step2Form.setValue('problem', scopeData.problematique);
        step2Form.setValue('kpis', scopeData.kpis);
        step2Form.setValue('outils', scopeData.outils);
        
        setObjectifs(scopeData.objectives.length > 0 ? scopeData.objectives : ['', '']);
        setContraintes(scopeData.contraintes.length > 0 ? scopeData.contraintes : ['']);
  ...
  const handleStep2Submit = async (data: any) => {
    ...
    try {
      ...
      const missionData: CreateMissionData = {
        ...
        objectif: JSON.stringify(objectifs.filter(o => o.trim())), // Send objectives as JSON string
        problem: data.problem,
        contraintes: JSON.stringify(contraintes.filter(c => c.trim())),
        kpis: data.kpis,
        outils: data.outils,
        user_id: user?.id,
      };
